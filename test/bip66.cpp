...
TEST(bip66, check) {
  ...
  ASSERT_EQ(matches, testCases::encoded.size());
}
...
TEST(bip66, encode_from_pair) {
  ...
  ASSERT_EQ(matches, testCases::decoded.size());
}
...
TEST(bip66, should_be_valid_der) {
  ...
  ASSERT_EQ(matches, testCases::valid::der.size());
}
...
TEST(bip66, should_be_valid_pair) {
  ...
  for (auto& e : testCases::valid::r) {
    ...
    const auto wasEncoded = BIP66::encode(
        e,
  ...
  ASSERT_EQ(matches, testCases::valid::r.size());
}

/**/

TEST(bip66, should_be_invalid_encode) {
  auto misses = 0;
  ...
  ASSERT_EQ(misses, testCases::invalid::encoded::r.size());
}

/**/

TEST(bip66, should_be_invalid_decode) {
  auto misses = 0;
  for ( auto& e : testCases::invalid::decoded::der) {
    !BIP66::check(e) ? ++misses : 0;
  };
  ASSERT_EQ(misses, testCases::invalid::decoded::der.size());
}
