...
void testDecoding() {
  ...
  for (auto i = 0; i < count; ++i) {
    ...
    const auto decodable = BIP66::decode(encodedBytes, r, s);
    ...
    if (decodable && rawSignatureBuffer ==TestCases().decoded[i]) {
      ++matches;
    };
  };
  char buf[56];
...
void testEncodingPair() {
  ...
  auto count = TestCases().decoded.size();
  auto matches = 0;
  for (auto i = 0; i < count; ++i) {
    std::vector<uint8_t> decodedBytes = TestCases().decoded[i];
    std::vector<uint8_t> r(32);
    std::vector<uint8_t> s(32);
    ...
    const auto encodable = BIP66::encode(r, s, rawSignatureBuffer);
    
    if (encodable && rawSignatureBuffer == TestCases().encoded[i] && (BIP66::check(rawSignatureBuffer))) {
      ++matches;
    };
  };
  char buf[56];
...
void testEncodingRaw() {
  ...
  for (auto i = 0; i < count; ++i) {
    ...
    const auto encodable = BIP66::encode(&decodedBytes[0], signatureBuffer);
    
    if (encodable && signatureBuffer == TestCases().encoded[i] && (BIP66::check(signatureBuffer))) {
      ++matches;
    };
  };
  char buf[56];
...
void testEncodeDecode() {
  ...
  for (auto i = 0; i < count; ++i) {
    ...
    const auto reencoded = BIP66::encode(r, s, temp);

    (checked && decoded && reencoded) ? ++matches : 0;
  };
  char buf[56];
...
void testShouldBeValid() {
  ...
  auto count = TestCases::valid().der.size();
  auto matches = 0;
  for (auto i = 0; i < count; ++i) {
    std::vector<uint8_t> encodedBytes = TestCases::valid().der[i];
    ...
    std::vector<uint8_t> temp(72);
    const auto reencodable = BIP66::encode(r, s, temp);

    (checked && decodable && reencodable) ? ++matches : 0;
  };
  char buf[56];
    sprintf(buf, "\n%d out of %d successful!\n", matches, count);
    Serial.print(buf);
...
void testShouldBeValidPair() {
  ...
  auto count = TestCases::valid().r.size();
  auto matches = 0;
  for (auto i = 0; i < count; ++i) {
    std::vector<uint8_t> r = TestCases::valid().r[i];
    std::vector<uint8_t> s = TestCases::valid().s[i];

    std::vector<uint8_t> compactSignature(64);
    const auto encodable = BIP66::encode(r, s, compactSignature);

    if (encodable && (BIP66::check(compactSignature))) {
      ++matches;
    };
  };
  char buf[56];
...
void testShouldBeInvalidEncode() {
  ...
  auto count = TestCases::invalid::encode().r.size();
  auto misses = 0;
  for (auto i = 0; i < count; ++i) {
    std::vector<uint8_t> encodedR = TestCases::invalid::encode().r[i];
    std::vector<uint8_t> encodedS = TestCases::invalid::encode().s[i];
    std::vector<uint8_t> signature; 

    const auto isInvalid = !BIP66::encode(encodedR, encodedS, signature) || !BIP66::check(signature);
    isInvalid ? ++misses : 0;
  };
  char buf[56];
    sprintf(buf, "\n%d out of %d successful!\n", misses, count);
...
void testShouldBeInvalidDecode() {
  ...
  auto count = TestCases::invalid::decode().der.size();
  auto misses = 0;
  for (auto i = 0; i < count; ++i) {
    std::vector<uint8_t> encodedBytes = TestCases::invalid::decode().der[i];
    !BIP66::check(encodedBytes) ? ++misses : 0;
  };
  char buf[56];
    sprintf(buf, "\n%d out of %d successful!\n", misses, count);
    Serial.print(buf);
